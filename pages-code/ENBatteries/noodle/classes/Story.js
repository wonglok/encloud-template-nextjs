...
export class Story {
  ...
  noodleGeoMkaer({
    ...
    let colorsArray = [];
    let tempColor = new Color("#ffffff");
    let palete = colors[17];
    // unroll colors
    for (let i = 0; i < posArray.length; i++) {
      tempColor.setStyle(palete[i % palete.length]);
      // tempColor
      colorsArray[i * 3 + 0.0] = tempColor.r;
      colorsArray[i * 3 + 1.0] = tempColor.g;
      colorsArray[i * 3 + 2.0] = tempColor.b;
    }
    ...
    lineGeo.setAttribute(
      "colors",
      new InstancedBufferAttribute(colorsArray, 3)
    );
    ...
    let ballBaseGeo = new IcosahedronBufferGeometry(
      0.03 * this.parent.momoScale * 2.0,
      2
    );
  ...
  prepAnimation() {
    ...
    let updateCtrlPts = () => {
      for (let eachCtrlPt = 0; eachCtrlPt < ctrlPts; eachCtrlPt++) {
        for (let i = 0; i < Math.pow(count, 0.5); i++) {
          for (let k = 0; k < Math.pow(count, 0.5); k++) {
            let phi = (i / Math.pow(count, 0.5)) * Math.PI * 2.0;
            let theta = (k / Math.pow(count, 0.5)) * Math.PI * 2.0;
            let cp = eachCtrlPt / ctrlPts;

            ball3.setFromSphericalCoords(
              (0.2 + easing.linear(cp)) *
                30.0 *
                this.inverseScaleForFloatingPts,
              phi,
              theta
            );

            ball3.applyAxisAngle(xaxis, (1 + cp) * Math.PI * 2.0);

            ball3.applyAxisAngle(yaxis, (1 + cp) * Math.PI * 2.0);

            ball3.applyAxisAngle(zaxis, (1 + cp) * Math.PI * 2.0);

            this[`controlPoint${eachCtrlPt}`].push(ball3.x, ball3.y, ball3.z);
    ...
    let getLineMat = () => {
      return new RawShaderMaterial({
        ...
        fragmentShader: `
          ...
          void main (void) {
            gl_FragColor = vec4(vColorTemp, 0.7 * vT);
          }
        `,
        defines: {
    ...
    let getBallMat = () => {
      return new ShaderMaterial({
        ...
        fragmentShader: `
          ...
          void main (void) {
            gl_FragColor = vec4(vColorTemp, 0.7);
          }
        `,
        defines: {
          // lengthSegments: subdivisions.toFixed(1)
