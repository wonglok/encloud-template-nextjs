...
export async function getStaticProps(context) {
  let buildTimeCache = await fetch(
    `${BASEURL_REST}/project?action=get-one-of-published`,
    {
      ...
      body: JSON.stringify({ _id: "60a6ebc7d80d490008f8ab95" }),
      method: "POST",
      mode: "cors",
    }
  )
...
let loadProjectJSON = ({ buildTimeCache }) => {
  console.log("buildTimeCache", buildTimeCache);
  return (
    buildTimeCache || {
      published: true,
...
function EffectNode({ buildTimeCache }) {
  ...
  useEffect(() => {
    let enRunTime = new ENRuntime({
      projectJSON: loadProjectJSON({ buildTimeCache }),
